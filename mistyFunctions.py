...
class Robot:
    ...
    def playAudio(self,file_name):
        if file_name in self.audio_saved:
            requests.post('http://' + self.ip + '/api/audio/play',json={"AssetId": file_name, 'Volume': 100})
        else:
            print(file_name,"not found on the robot, use <robot_name>.printAudioList() to see the list of saved audio files")

    def recordAudio(self):
        # requests.post('http://' + self.ip + '/api/audio/speech/capturevosk', json={'CaptureSpeech': True, 'MaxSpeechLength': 20000, 'SilenceTimeout': 10000, 'CaptureFile': True})
        requests.post('http://' + self.ip + '/api/audio/speech/capturevosk')

    def getAudioFile(self, file_name):
        resp = requests.get('http://' + self.ip + '/api/audio?FileName=' + file_name, json={'Base64':True})
